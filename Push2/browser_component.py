# Embedded file name: c:\Jenkins\live\output\win_32_static\Release\python-bundle\MIDI Remote Scripts\Push2\browser_component.py
    ...
    def _focus_list_with_index(self, index, crop = True):
        ...
        if self._focused_list_index != index:
            ...
            if not 0 <= index < len(self._lists):
                ...
                self._focused_list_index == len(self._lists) - 1 and self._replace_preview_list()
            self._load_neighbour_overlay.set_enabled(False)
            self._update_navigation_buttons()
            return True
        else:
