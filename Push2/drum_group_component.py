# Embedded file name: c:\Jenkins\live\output\win_32_static\Release\python-bundle\MIDI Remote Scripts\Push2\drum_group_component.py
...
class DrumGroupComponent(DrumGroupComponentBase):
    ...
    def _chain_color_for_pad(self, pad, color):
        if not (color == 'DrumGroup.PadFilled' and pad.chains and liveobj_valid(pad.chains[0])):
            raise AssertionError('Filled pads should have a chain')
            color = IndexedColor.from_live_index(pad.chains[0].color_index)
        elif not (color == 'DrumGroup.PadMuted' and pad.chains and liveobj_valid(pad.chains[0])):
            raise AssertionError('Filled pads should have a chain')
