# Embedded file name: c:\Jenkins\live\output\win_32_static\Release\python-bundle\MIDI Remote Scripts\Push2\firmware.py
...
class FirmwareVersion(object):
    ...
    def __cmp__(self, other):
        if other is None:
            ...
            return 0
        elif self.major > other.major or self.major == other.major and self.minor > other.minor or self.major == other.major and self.minor == other.minor and self.build > other.build:
            return 1
        else:
            return -1

    def __repr__(self):
        return '<FirmwareVersion %i.%i.%i %s>' % (self.major,
         self.minor,
         self.build,
         self.release_type)
